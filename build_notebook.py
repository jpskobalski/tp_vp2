...
cells.append(code(
    ...
    "\n"
    "def pick_device():\n"
    "    \"\"\"Devuelve el mejor device disponible: CUDA (NVIDIA) > MPS (Apple Silicon) > CPU.\"\"\"\n"
    ...
    "\n"
    "DEVICE = pick_device()\n"
    "# EasyOCR solo soporta CUDA o CPU (no MPS) — fallback a CPU en Mac\n"
    ...
    "elif DEVICE == 'mps':\n"
    "    print('Apple MPS    : disponible (entrenamiento acelerado en M-series)')\n"
    "else:\n"
    "    print('Modo CPU     : entrenamiento lento — usar solo para validar el pipeline')\n"
    "print('EasyOCR usará:', 'GPU (CUDA)' if EASYOCR_GPU else 'CPU')\n"
    "\n"
    "SEED = 42\n"
...
cells.append(md(
    "### 8.1. Guardar predicciones en formato estándar para comparación\n"
    "\n"
    "Para poder comparar contra otros modelos (EfficientNet, OCR+regex, etc.) en "
    "`comparison.ipynb`, guardamos las predicciones con un schema único:\n"
    "`image_path, label, score, pred, model_name`."
))

cells.append(code(
    "MODEL_NAME = f'yolov8{MODEL_SIZE}'\n"
    "OUT_DIR_PRED = WORK_DIR / MODEL_NAME\n"
    "OUT_DIR_PRED.mkdir(parents=True, exist_ok=True)\n"
    "\n"
    "df_preds = pd.DataFrame({\n"
    "    'image_path': df_test['image_path'].values,\n"
    "    'label':      y_true.astype(int),\n"
    "    'score':      scores.astype(float),\n"
    "    'pred':       y_pred.astype(int),\n"
    "    'model_name': MODEL_NAME,\n"
    "})\n"
    "df_preds.to_csv(OUT_DIR_PRED / 'test_predictions.csv', index=False)\n"
    "print(f'Guardado: {OUT_DIR_PRED / \"test_predictions.csv\"}  ({len(df_preds)} filas)')\n"
    "df_preds.head()\n"
))

cells.append(md(
    "### 8.2. Barrido de umbral\n"
    "\n"
