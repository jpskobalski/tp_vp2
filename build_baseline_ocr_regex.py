"""
Genera baseline_ocr_regex.ipynb — baseline "naive" sin detector entrenado.

Pipeline:
  Imagen completa → EasyOCR full-image → regex de contactos → predicción binaria

Sirve como baseline para responder en el paper:
  "¿Necesitamos un detector? ¿No alcanza con correr OCR sobre la imagen entera?"

Métricas que produce:
  - precision / recall / F1 / AUC-PR sobre test
  - latencia promedio por imagen (para tabla del paper)
  - test_predictions.csv en formato estándar (compatible con comparison.ipynb)
"""
...
cells.append(md(
    ...
    "\n"
    "**Propósito:** evaluar la alternativa más simple al detector entrenado: correr "
    "OCR sobre la imagen completa y filtrar con regex. Sin detector, sin entrenamiento.\n"
    "\n"
    "**Por qué importa para el paper:** es el baseline que responde *\"¿realmente "
    "necesitamos YOLO?\"*. Si esta baseline es competitiva en accuracy, perdemos parte "
    "de la justificación de YOLO. Si es lenta o tiene muchos falsos positivos, "
    "queda claro el aporte del detector.\n"
    "\n"
    "**Métricas a reportar:** precision / recall / F1 / AUC-PR en test + latencia "
    "promedio (que será claramente peor que YOLO + OCR-sobre-crops).\n"
))
...
# 2. Configuración
# ============================================================================
cells.append(md("## 2. Configuración"))
...
# ============================================================================
# 3. Regex (idénticos a los de tp_final.ipynb — clave para que la comparación sea justa)
# ============================================================================
cells.append(md(
    "## 3. Regex de detección de contactos\n"
    "\n"
    "**Importante:** usamos *exactamente los mismos regex* que en `tp_final.ipynb` para "
    "que la comparación sea justa. Si esta baseline es mejor que YOLO, no es porque "
    "tenga acceso a información extra — es porque OCR-full-image captura cosas que "
    "YOLO no detecta (o porque las pseudo-labels de YOLO eran ruidosas)."
))
...
# ============================================================================
# 4. OCR full-image con caché
# ============================================================================
cells.append(md(
    "## 4. OCR sobre imagen completa (con caché)\n"
    "\n"
    "Procesa cada imagen del test set con EasyOCR. Guarda los resultados en "
    "`work/ocr_cache/test_ocr.json` para no tener que reprocesar.\n"
    "\n"
    "**Atención CPU:** ~1-2s por imagen en CPU = 50-100 min para 3000 imágenes. "
    "Subir `MAX_TEST_IMAGES` gradualmente si querés validar primero."
))
...
# ============================================================================
# 5. Aplicar regex sobre OCR → predicción por imagen
# ============================================================================
cells.append(md(
...
# 6. Métricas
# ============================================================================
cells.append(md("## 6. Métricas en test"))
...
# ============================================================================
# 7. Guardar predicciones en formato estándar
# ============================================================================
cells.append(md(
    "## 7. Guardar predicciones para comparación\n"
    "\n"
    "Schema: `image_path, label, score, pred, model_name`. Compatible con "
    "`comparison.ipynb`."
))
...
# ============================================================================
cells.append(md("## 8. Inspección visual de errores"))
...
OUT = Path(__file__).parent / 'baseline_ocr_regex.ipynb'
nbf.write(nb, OUT)
